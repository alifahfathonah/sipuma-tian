...
|-----------------------------------------------------------------------------------------------------------------
*/
function get_all_berita($per_pg,$offset){
		$this->db->where('category = "berita" and status = 1');
		$this->db->order_by('id','desc');
		$query = $this->db->get('artikel',$per_pg,$offset);
		return $query->result();
	}
...
function berita_count(){
		return $this->db->count_all('artikel where category = "berita" and status = "1" ');
	}
	/*
|-----------------------------------------------------------------------------------------------------------------
| END BERITA DETAIL
